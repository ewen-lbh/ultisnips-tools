...
(function(){
  var c, el, els, id, ids, snippet, updateResult, listenToAll, updateSnippetObject, insertAtCursor, disableOtherInputs, disableButton, enableButton, disableInsertTabstopButton, disableInsertTriggerRegexGroupButton, ordinalSuffix, uaparser, ua;
  c = console.log;
  ...
  */
  els('.ordinal[data-source]').forEach(function(it){
    var targetElement, sourceElement;
    targetElement = it;
    sourceElement = el(it.dataset.source);
  ...
  */
  ...
  if (ua.browser.name === 'Firefox' && +ua.browser.major < 79) {
    id('browser-not-supported').style.display = 'block';
  } else {
    id('browser-not-supported').style.display = 'none';
  }
  els('span[data-key=detectedBrowser]').forEach(function(it){
